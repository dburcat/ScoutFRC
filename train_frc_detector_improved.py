#!/usr/bin/env python3
"""
Improved FRC detector training with a GPU-first configuration.

This keeps the stronger training recipe from the reference notes while
preferring CUDA when a compatible PyTorch build is available. When run inside
the Python 3.11 GPU environment, Ultralytics can use the NVIDIA GPU instead of
falling back to CPU.
"""
...
# Model configuration
MODEL_SIZE = "m"  # Medium model from the reference notes
TRAINING_EPOCHS = 150
PATIENCE = 30
IMG_SIZE = 640
...
BATCH_SIZE = -1 if str(DEVICE).startswith("cuda") else 16
...
def main():
    ...
    print(f"Device: {DEVICE}")
    if DEVICE == "cpu":
        print("GPU not active. Use the Python 3.11 GPU environment with CUDA PyTorch to train on NVIDIA.")
